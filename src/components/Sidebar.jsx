    ...
    <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      <div className="sidebar-header">
        <div className="logo">
          ...
          {!collapsed && <span className="logo-text">ResumeCraft</span>}
        </div>
        <button 
      ...
      <nav className="sidebar-nav">
        ...
        <div className="nav-section">
          <span className="nav-section-title">模板与样式</span>
          {settingsTabs.map(tab => (
            <button
              key={tab.id}
              className={`nav-item ${activeTab === tab.id && showSettings ? 'active' : ''}`}
              onClick={() => { setActiveTab(tab.id); setShowSettings(true); }}
              title={tab.label}
            >
              <tab.icon size={18} />
              {!collapsed && <span>{tab.label}</span>}
      ...
      <div className="sidebar-footer">
        <button className="action-btn" onClick={onExport} title="导出简历">
          <Download size={18} />
